...
var (
	...
	dockerInsecureSkipVerifyTLS     = dockerScan.Flag("insecure-skip-verify-tls", "Skip TLS verification for insecure registries").Envar("DOCKER_INSECURE_SKIP_VERIFY_TLS").Bool()
...
func runSingleScan(ctx context.Context, cmd string, cfg engine.Config) (metrics, error) {
	...
	case dockerScan.FullCommand():
		...
		cfg := sources.DockerConfig{
			...
			InsecureSkipVerifyTLS: *dockerInsecureSkipVerifyTLS,
		}
